...
from scripts.model.embeddings import EmbeddingGenerator, ChunkLinker, SubgraphBuilder, SubgraphSummarizer, HierarchicalSummarizer
...
def pipeline(embedding_model_name: str = "paraphrase-multilingual-mpnet-base-v2"):
    ...
    root_summary, root_connections = hierarchical_summarizer.create_hierarchical_summaries(initial_summaries)

    print(f"\n=== Hierarchical Summarization Complete ===")
    print(f"Total summary levels created: {len(set(s['level'] for s in hierarchical_summarizer.all_summaries))}")
    print(f"Total summaries: {len(hierarchical_summarizer.all_summaries)}")
    print(f"Parent-child relations: {len(hierarchical_summarizer.parent_child_relations)}")
    ...
    for level in sorted(set(s['level'] for s in hierarchical_summarizer.all_summaries)):
        level_summaries = [s for s in hierarchical_summarizer.all_summaries if s['level'] == level]
    ...
    for level in sorted(set(s['level'] for s in hierarchical_summarizer.all_summaries)):
        ...
        if len(level_summaries) > 1:
            ...
            if summary_link_objects:
                ...
                print(f"Created {len(summary_link_objects)} similarity edges for level {level}")
            else:
    ...
    if hierarchical_summarizer.parent_child_relations:
        db_client.create_hierarchical_edges(hierarchical_summarizer.parent_child_relations)
        print(f"Created {len(hierarchical_summarizer.parent_child_relations)} hierarchical edges")
    ...
    if root_summary:
        ...
        print(f"Created root node and connected {len(root_connections)} top-level summaries")

    level_1_summaries = [s for s in hierarchical_summarizer.all_summaries if s['level'] == 1]
    for summary in level_1_summaries:
        ...
        for chunk_embedding in cluster_embeddings:
            similarity = generator.calculate_similarity(chunk_embedding, summary_embedding)
    ...
    for conn in high_similarity[:5]:
        print(f"  {conn['node1']} ↔ {conn['node2']}: {conn['weight']:.3f} ({conn['relationship_type']})")
    
    centrality = db_client.calculate_node_centrality()
    print(f"\nTop 5 most connected nodes by weight:")
    for node in centrality[:5]:
        print(f"  {node['node_id']}: total_weight={node['total_weight']:.3f}, connections={node['total_connections']}")
